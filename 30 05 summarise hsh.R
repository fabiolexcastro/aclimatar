...
rstr.bsln <- map(.x = 1:length(year.bsln), .f = function(i){
  ...
  names(rstr) <- glue('HSH_{year.bsln[i]}')
  return(rstr)
...
names(rstr.bsln) <- glue('HSH_bsl')

##
terra::writeRaster(x = rstr.bsln, filename = glue('./common_data/indices_average/historical/HSH/HSH_raw.tif'), overwrite = TRUE)
...
smmr.gcme <- function(gcme){
  ...
  fles <- grep('HSH_max', fles, value = T)
  year <- 2025:2055
  ...
  ##
  rstr <- mean(rstr)
  names(rstr) <- glue('HSH_{sspe}_{gcme}')
  
  ##
  dout <- glue('./common_data/indices_average/{sspe}_{gcme}/HSH')
  dir_create(dout)
  terra::writeRaster(x = rstr, filename = glue('./common_data/indices_average/{sspe}_{gcme}/HSH/HSH_raw.tif'), overwrite = TRUE)
  cat('Done!\n')
  return(rstr)
...
names(rstr.ftre.avrg) <- glue('HSH_{sspe}')
dir_create(glue('./common_data/indices_global/{sspe}'))
terra::writeRaster(x = rstr.ftre.avrg, filename = glue('./common_data/indices_global/{sspe}/HSH_raw.tif'))
