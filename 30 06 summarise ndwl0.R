...
##
fles.bsln <- dirs.bsln %>% dir_ls(., regexp = '.tif$') %>% as.character() %>% grep('/NDWL0-', ., value = T)
...
rstr.bsln <- map(.x = 1:length(year.bsln), .f = function(i){
  ...
  names(rstr) <- glue('NDWL0_{year.bsln[i]}')
  return(rstr)
...
names(rstr.bsln) <- glue('NDWL0_bsl')

##
terra::writeRaster(x = rstr.bsln, filename = glue('./common_data/indices_average/historical/NDWL0/NDWL0_raw.tif'), overwrite = TRUE)
...
smmr.gcme <- function(gcme){
  ...
  ##
  rstr <- mean(rstr)
  names(rstr) <- glue('NDWL0_{sspe}_{gcme}')
  
  ##
  dout <- glue('./common_data/indices_average/{sspe}_{gcme}/NDWL0')
  dir_create(dout)
  terra::writeRaster(x = rstr, filename = glue('./common_data/indices_average/{sspe}_{gcme}/NDWL0/NDWL0_raw.tif'), overwrite = TRUE)
  cat('Done!\n')
  return(rstr)
...
names(rstr.ftre.avrg) <- glue('NDWL0_{sspe}')
dir_create(glue('./common_data/indices_global/{sspe}'))
terra::writeRaster(x = rstr.ftre.avrg, filename = glue('./common_data/indices_global/{sspe}/NDWL0_raw.tif'))
