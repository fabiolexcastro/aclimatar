...
# Function
calc_ntx30 <- function(yr, mn, thr = 30){
  
  # yr <- '2021'; mn <- '01'
  
  outfile <- paste0(out_dir, 'NTX30-', yr, '-', mn, '.tif')
...
for(ssp in c('ssp245', 'ssp370')){
  ...
  for(prd in c('2021_2040', '2041_2060', '2061_2080', '2081_2100')){
    ...
    for(gcm in c('ACCESS-ESM1-5', 'MPI-ESM1-2-HR', 'EC-Earth3', 'INM-CM5-0', 'MRI-ESM2-0')){
      ...
      out_dir <- paste0(root, '/indices/', ssp, '_', gcm, '/NTX30/') 
      
      1:nrow(stp) %>% 
        purrr::map(.f = function(i){calc_ntx30(yr = stp$yrs[i], mn = stp$mnt[i], thr = 30)})
