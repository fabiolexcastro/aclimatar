...
# SpatExtent : -179.0625, 25.3125, -56.875, 85.625 (xmin, xmax, ymin, ymax)
...
install_github(c("SantanderMetGroup/climate4R.UDG"))
...
# -179.0625, 25.3125, -56.875, 85.625 (xmin, xmax, ymin, ymax)
# c(-179.0625, 25.3125, -56.876, 85.625)
...
downloadCMIP6 <- function(ds_name="CMIP6_ACCESS-ESM1-5_scenario_r1i1p1f1", rcp="ssp585", varname="pr", 
  ...
  fname_his <- paste0(basedir, "/", dataset.hist,"_",varname,"_America_daily.tif")
  ...
  #rcp data, file name
  fname_rcp <- paste0(basedir, "/", dataset.rcp,"_",varname,"_America_daily_",min(years.rcp), "-", max(years.rcp), ".tif")
...
for (i in 1:length(dataset_list)) {
  ...
  for (scenario in c("ssp245", "ssp370")) {
    for (varname in c("rsds")) {
      cmip6_data <- downloadCMIP6(ds_name=dataset_list[i], 
                                  rcp=scenario, 
                                  varname=varname, 
                                  years.hist=1995:2014, 
                                  years.rcp=2021:2060, #previously i had done 2021:2060
