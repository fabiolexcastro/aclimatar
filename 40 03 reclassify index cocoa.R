...
## Points datasets cocoa --------------------------------------------------

### Southamerica
pnts.col <- read_csv('//catalogue/workspace-cluster9/CL/v1/data/tbl/presences/occ_col.csv')
pnts.ecu <- vect('//catalogue/workspace-cluster9/CL/v1/data/shp/points/points_ecuador.shp')
pnts.per <- read_csv('//catalogue/workspace-cluster9/CL/v1/data/tbl/presences/occ_per.csv')

### Centralamerica
pnts.cam <- read_csv('//catalogue/workspace-cluster9/CAM/v1/data/tbl/points/6_occ_swd_rm.csv')
pnts.cam

## Join points
pnts.col <- pnts.col %>% mutate(iso = 'COL') %>% dplyr::select(lon, lat, iso)
pnts.ecu <- pnts.ecu %>% crds() %>% as_tibble() %>% mutate(iso = 'ECU') %>% dplyr::select(lon = x, lat = y, iso)
pnts.per <- pnts.per %>% mutate(iso = 'PER') %>% dplyr::select(lon = x, lat = y, iso)

pnts.cam <- mutate(pnts.cam, iso = terra::extract(zone, pnts.cam[,c('Lon', 'Lat')])$sov_a3)
pnts.cam <- dplyr::select(pnts.cam, lon = Lon, lat = Lat, iso)

pnts.ccoa <- rbind(pnts.col, pnts.ecu, pnts.per, pnts.cam)
write.csv(pnts.ccoa, './tble/points/points_cocoa.csv', row.names = FALSE)

# Points ------------------------------------------------------------------
pnts <- read_csv('./tble/points/points_cocoa.csv', show_col_types = FALSE)

pnts.pan <- vect('//alliancedfs.alliance.cgiar.org/CL9_Coffee_Cocoa2/_cocoa_cam_2/shp/occ/occ_cocoa_cam.shp')
pnts.pan$iso <- terra::extract(zone, pnts.pan)$sov_a3
pnts.pan <- pnts.pan[pnts.pan$iso == 'PAN',]
pnts.pan <- crds(pnts.pan)
pnts.pan <- as.data.frame(pnts.pan)
pnts.pan <- mutate(pnts.pan, iso = 'PAN')
pnts.pan <- rename(pnts.pan, lon = x, lat = y)

pnts <- rbind(pnts, pnts.pan)
...
stck <- reduce(stck, c)
terra::writeRaster(x = stck, filename = './common_data/indices_global/ssp370/index_rcl-cocoa.tif', overwrite = TRUE)
