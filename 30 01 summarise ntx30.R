...
rstr.bsln <- map(.x = 1:length(year.bsln), .f = function(i){
  ...
  names(rstr) <- glue('NTX30_{year.bsln[i]}')
  return(rstr)
...
names(rstr.bsln) <- glue('NTX30_bsl')

##
terra::writeRaster(x = rstr.bsln, filename = glue('./common_data/indices_average/historical/NTX30/NTX30_raw.tif'), overwrite = TRUE)
...
smmr.gcme <- function(gcme){
  ...
  ##
  rstr <- mean(rstr)
  names(rstr) <- glue('NTX30_{sspe}_{gcme}')
  
  ##
  dout <- glue('./common_data/indices_average/{sspe}_{gcme}/NTX30')
  dir_create(dout)
  terra::writeRaster(x = rstr, filename = glue('./common_data/indices_average/{sspe}_{gcme}/NTX30/NTX30_raw.tif'), overwrite = TRUE)
  cat('Done!\n')
  return(rstr)
...
names(rstr.ftre.avrg) <- glue('NTX30_{sspe}')
dir_create(glue('./common_data/indices_global/{sspe}'))
terra::writeRaster(x = rstr.ftre.avrg, filename = glue('./common_data/indices_global/{sspe}/NTX30_raw.tif'))
