...
rstr.bsln <- map(.x = 1:length(year.bsln), .f = function(i){
  ...
  names(rstr) <- glue('NDD_{year.bsln[i]}')
  return(rstr)
...
names(rstr.bsln) <- glue('NDD_bsl')

##
terra::writeRaster(x = rstr.bsln, filename = glue('./common_data/indices_average/historical/NDD/NDD_raw.tif'), overwrite = TRUE)
...
smmr.gcme <- function(gcme){
  ...
  ##
  rstr <- mean(rstr)
  names(rstr) <- glue('NDD_{sspe}_{gcme}')
  
  ##
  dout <- glue('./common_data/indices_average/{sspe}_{gcme}/NDD')
  dir_create(dout)
  terra::writeRaster(x = rstr, filename = glue('./common_data/indices_average/{sspe}_{gcme}/NDD/NDD_raw.tif'), overwrite = TRUE)
  cat('Done!\n')
  return(rstr)
...
names(rstr.ftre.avrg) <- glue('NDD_{sspe}')
dir_create(glue('./common_data/indices_global/{sspe}'))
terra::writeRaster(x = rstr.ftre.avrg, filename = glue('./common_data/indices_global/{sspe}/NDD_raw.tif'))
