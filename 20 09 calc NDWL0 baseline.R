...
calc_ndwl0 <- function(yr, mn){
  ...
  if (!file.exists(outfile)) {
    ...
    # Files
    # t1 <- system.time({
    pr_fls <- paste0(pr_pth,'/Prec_',yr, '-', as.numeric(mn), '.tif')
    pr_fls <- pr_fls[file.exists(pr_fls)]
    tx_fls <- paste0(tx_pth,'/Tmax_', yr, '-', as.numeric(mn),'.tif')
    tx_fls <- tx_fls[file.exists(tx_fls)]
    tm_fls <- paste0(tm_pth,'/Tmin_', yr, '-', as.numeric(mn),'.tif')
...
out_dir <- paste0(root,'/indices/historical/NDWL0')
1:nrow(stp) %>%
  purrr::map(.f = function(i){
