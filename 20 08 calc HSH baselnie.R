...
calc_hsh <- function(yr, mn){
  ...
  if(sum(file.exists(c(outfile1,outfile2,outfile3))) < 2){
    ...
    if(as.numeric(yr) > 2020 & mn == '02'){
      ...
      dts2 <- seq(from = as.Date(paste0(yr,'-',mn,'-01')), to = as.Date(paste0(yr,'-',mn,'-',last_day)), by = 'day')
    }
    # Files
    tx_fls <- paste0(tx_pth,'/','/Tmax_',dts,'.tif')
    tx_fls <- tx_fls[file.exists(tx_fls)]
    tm_fls <- paste0(tm_pth,'/','/Tmin_',dts,'.tif')
    ...
    if(as.numeric(yr) > 2020){
      ...
      rh_fls <- paste0(rh_pth,'/','/RH.',gsub(pattern='-', replacement='.', x=dts2, fixed=T),'.tif')
    }
...
out_dir <-paste0(root,'/indices/historical', '/HSH')
